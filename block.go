...
const maxBlockSize = 128 * 1024 // 128 KB
...
func (b *Block) SetHash() {
	timeBytes := ConvertTimestampToByte(b.Timestamp)
...
func (block *Block) PrintTransaction() {
	for idx, transaction := range block.Transactions {
		...
		fmt.Printf("-- Created timestamp (UTC+0): %s\n", GetTimestampFormat(transaction.Timestamp))
		fmt.Printf("-- Data: %x\n", transaction.Data)
	}
}
